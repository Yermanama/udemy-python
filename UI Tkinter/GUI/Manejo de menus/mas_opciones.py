import tkinter as tk
from tkinter import ttk, messagebox, Menu

ventana = tk.Tk()
ventana.geometry('500x500')
ventana.title('Mas opciones de menu')
ventana.iconbitmap(r'C:\Users\marti\OneDrive\Escritorio\Programación\udemy_python\UI Tkinter\Manejo de menus\menus.py')

entrada1 = ttk.Entry(ventana, width=30)
entrada1.grid(row=0, column=0)
etiqueta = ttk.Label(ventana, text='Esto es una etiqueta')
etiqueta.grid(row=1, column=0,columnspan=2)


def mostrar_mensaje():
    mensaje = entrada1.get()
    etiqueta.config(text=entrada1.get())
    if mensaje:
        messagebox.showinfo(mensaje)


def crear_menu():
    menu_principal = Menu(ventana)
    sub_archivo = Menu(menu_principal, tearoff=0)
    sub_archivo.add_command(label='Nuevo')
    # Podemos añadir separadores si queremos
    sub_archivo.add_separator()
    # Creamos otro comando
    sub_archivo.add_command(label='Salir')
    # Creamos otro menu llamado ayuda
    sub_ayuda = Menu(menu_principal, tearoff=0)
    sub_ayuda.add_command(label='Acerca de')
    # Ahora añadimos los dos submenus al grande
    menu_principal.add_cascade(menu=sub_archivo, label='Archivo')
    menu_principal.add_cascade(menu=sub_ayuda, label='Ayuda')
    # Mostramos el menu en le ventana principal
    ventana.config(menu=menu_principal)


crear_menu()

boton = ttk.Button(ventana, text='Mostrar mensaje', command=mostrar_mensaje)

ventana.mainloop()